...
class ExportVirtualVisitUatMatrix extends Command
{
    protected $signature = 'iwosan:export-virtual-visit-uat-matrix {--output= : Absolute or relative output CSV path}';
    protected $description = 'Export virtual-visit UAT execution matrix CSV from virtual_visit_workflow config.';
    ...
    public function handle(): int
    {
        ...
        if ($statuses === []) {
            $this->error('No virtual statuses found in config(virtual_visit_workflow.statuses).');
            return self::FAILURE;
        }

        $defaultPath = base_path('../execution_plans/VIRTUAL_VISIT_UAT_EXECUTION_MATRIX.csv');
        ...
        $this->info("Virtual-visit UAT execution matrix exported: {$resolvedPath}");
